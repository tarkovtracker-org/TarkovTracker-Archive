...
describe('config/features', () => {
  ...
  describe('API_FEATURES constant', () => {
    it('should default all features to false when no environment variables are set', async () => {
      ...
      vi.doMock('../../src/config/features', async () => {
        const actual = await vi.importActual('../../src/config/features');
        return actual;
      });
      const { API_FEATURES: freshFeatures } = await import('../../src/config/features');
    ...
    it('should handle false value for FEATURE_ERROR_HANDLING', () => {
    ...
    it('should ignore invalid values for FEATURE_ERROR_HANDLING', () => {
