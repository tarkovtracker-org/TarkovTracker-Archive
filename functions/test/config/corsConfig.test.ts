...
describe('config/corsConfig', () => {
  ...
  describe('validateOrigin', () => {
    ...
    describe('without allowedOrigins (pattern-based validation)', () => {
      ...
      it('should be case insensitive for dangerous patterns', () => {
        ...
        expect(validateOrigin('LOCALHOST')).toBe('localhost'); // Normalized and allowed in dev, blocked in prod
        expect(validateOrigin('LOCALHOST')).toBe('localhost'); // Normalized and allowed in dev, blocked in prod
        expect(validateOrigin('HTTP://LOCALHOST')).toBe(false); // Invalid protocol
      });
    });
  });
  ...
  describe('getExpressCorsOptions', () => {
    ...
    it('should call validateOrigin with correct parameters', (done) => {
      const corsOptions = getExpressCorsOptions();
      // Create a mock callback
      const callback = vi.fn((err, allow) => {
        // The callback may be called with an error for validation, let's check what actually happens
        if (err) {
    ...
    it('should call callback with error for blocked origins', (done) => {
      ...
      const callback = vi.fn((err, allow) => {
        expect(err).toBeInstanceOf(Error);
        expect(err?.message).toBe('Not allowed by CORS');
        // allow parameter may or may not be provided depending on CORS implementation
        done();
      });
      // Test with a blocked origin
      corsOptions.origin!('localhost', callback);
    });
  });
