...
import { firestoreMock, seedDb, resetDb } from '../setup';
 ...
 *
 * Notes on test infra:
 * - Firestore mocks provide a transaction implementation with conflict simulation
 * - Use vi.doMock to control data loaders (getTaskData/getHideoutData) per test
 * - Capture transaction.update calls by stubbing runTransaction with injected transaction
 * - Avoid verifying nested Firestore state due to dot-path update mock; validate update inputs and call counts
 */
...
vi.doMock('../../src/utils/dataLoaders', async () => {
  ...
  return {
    ...
    getTaskData: vi.fn().mockResolvedValue({ tasks: [MOCK_TASKS.TASK_ALPHA, MOCK_TASKS.TASK_BETA] }),
  };
});
...
describe('ProgressService concurrency and transactions', () => {
  ...
  beforeEach(async () => {
    ...
    vi.mocked(getTaskData).mockResolvedValue({ tasks: [MOCK_TASKS.TASK_ALPHA, MOCK_TASKS.TASK_BETA] });
  ...
  it('applies consistent state under concurrent single-task updates to the same task', async () => {
    ...
    expect(firestoreMock.runTransaction).toHaveBeenCalledTimes(5);
  ...
  it('fails and rolls back when progress doc does not exist (non-retryable), leaving state unchanged', async () => {
    ...
    await expect(service.updateSingleTask(userId, taskId, 'completed', 'pvp')).rejects.toHaveProperty(
      'message',
   ...
   */
  const withCapturedTransaction = (fn: (transactionUpdate: any, transactionGet: any) => Promise<void>) => {
  ...
  it('handles chained dependency resolution under concurrent updates to the same parent task', async () => {
    ...
    const taskData = {
      tasks: [
        ...
        { id: 'task-b', alternatives: [], taskRequirements: [{ task: { id: 'task-a' }, status: ['complete'] }] },
  ...
  it('does not deadlock on circular dependencies and completes updates', async () => {
    ...
    const circularTaskData = {
      tasks: [
        { id: 'task-x', alternatives: [], taskRequirements: [{ task: { id: 'task-y' }, status: ['complete'] }] },
        { id: 'task-y', alternatives: [], taskRequirements: [{ task: { id: 'task-x' }, status: ['complete'] }] },
      ],
    };
    const { getTaskData } = await import('../../src/utils/dataLoaders');
    vi.mocked(getTaskData).mockResolvedValue(circularTaskData);

    await expect(service.updateSingleTask(userId, 'task-x', 'completed', 'pvp')).resolves.toBeUndefined();
  ...
  it('retries transactions deterministically when conflicts are forced and ultimately succeeds', async () => {
    ...
    await expect(service.updateSingleTask(userId, 'task-alpha', 'completed', 'pvp')).resolves.toBeUndefined();
  ...
  it('ensures partial updates do not persist on transaction failure', async () => {
    ...
    await expect(service.updateSingleTask(userId, 'task-alpha', 'completed', 'pvp')).rejects.toHaveProperty(
      'message',
      'Failed to update task'
    );

    // Validate that no progress doc was created as a side effect
    // Using ServiceTestHelpers to assert lack of document creation indirectly by absence of updates
    const progressCollection = firestoreMock.collection('progress');
    const docRef = progressCollection.doc(userId);
  ...
  it('handles mixed state updates (completed vs failed) to the same task concurrently', async () => {
    ...
    expect(firestoreMock.runTransaction).toHaveBeenCalledTimes(3);
