...
describe('Encryption Service', () => {
  ...
  describe('Key Generation and Derivation', () => {
    it('should generate random salt when none exists', async () => {
      ...
      mockCrypto.subtle.importKey.mockResolvedValue(mockKey);
      mockCrypto.subtle.deriveKey.mockResolvedValue(mockKey);
      // Clear previous calls and reset mock to default behavior
      ...
      // Should be called twice: once for salt (16 bytes) and once for IV (12 bytes)
      expect(mockCrypto.getRandomValues).toHaveBeenCalledTimes(2);
      // Check that it was called with a 16-byte array (the specific values don't matter)
    ...
    it('should derive consistent key from browser characteristics', async () => {
      ...
      expect(mockCrypto.subtle.deriveKey).toHaveBeenCalledWith(
        ...
        expect.any(Object), // CryptoKey is an object
  ...
  describe('Encryption/Decryption Round-trip', () => {
    ...
    beforeEach(() => {
      ...
      // Store original data for round-trip testing
      ...
      mockCrypto.subtle.decrypt.mockImplementation(
        (_params: any, _key: CryptoKey, _data: ArrayBuffer) => {
          // Return the original data that was stored during encryption
          if (originalData) {
    ...
    it('should handle JSON objects', async () => {
      ...
      mockCrypto.subtle.encrypt.mockImplementation(
        (_params: any, _key: CryptoKey, data: ArrayBuffer) => {
          originalData = new Uint8Array(data);
          return Promise.resolve(new Uint8Array(new Uint8Array(data).byteLength + 20));
        }
      );
      mockCrypto.subtle.decrypt.mockImplementation(
        (_params: any, _key: CryptoKey, _data: ArrayBuffer) => {
    ...
    it('should handle arrays', async () => {
      ...
      // Note: JSON.stringify converts undefined to null, so we expect that behavior
      const expectedArray = [
        'string',
        123,
        true,
        ...
        ['nested', 'array'],
        null,
        null, // undefined becomes null after JSON.stringify/parse cycle
      ];
    ...
    it('should generate unique IV for each encryption', async () => {
      ...
      mockCrypto.getRandomValues.mockImplementation((arr) => {
        callCount++;
        if (callCount === 1) {
          // First call for salt generation
          for (let i = 0; i < arr.length; i++) {
            arr[i] = i;
          }
        } else if (callCount === 2) {
          // Second call for first IV
          arr.set(firstIV);
        } else if (callCount === 3) {
          // Third call for second IV
          arr.set(secondIV);
        }
        return arr;
      });
      ...
      // Should be called 3 times: salt (16 bytes), first IV (12 bytes), second IV (12 bytes)
  ...
  describe('Error Handling', () => {
    ...
    it('should handle wrong password/key scenarios', async () => {
      ...
      mockLocalStorage.clear();
      // Mock decrypt to fail for wrong key scenario
      mockCrypto.subtle.decrypt.mockRejectedValue(new Error('Authentication failed'));
    ...
    it('should handle malformed JSON during decryption', async () => {
      ...
      // Should throw when trying to decode invalid UTF-8 - TextDecoder should handle this
      // but we need to mock it to throw an error for this test
      const originalTextDecoder = globalThis.TextDecoder;
      globalThis.TextDecoder = class extends originalTextDecoder {
        decode(_input?: any, _options?: any): string {
