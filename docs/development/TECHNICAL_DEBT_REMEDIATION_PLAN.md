...
|---------------|--------|--------|-------------------|
| `frontend/src/utils/errorHandler.ts` | ✅ Commit | Complete, useful utility | ✓ Review complete implementation  ✓ Verify imports in handlers  → `git add && git commit` |
| `functions/openapi/` | ✅ Commit | OpenAPI generation support | ✓ Check directory structure  ✓ Verify generation scripts  → `git add && git commit` |
| `functions/src/app/` | ⚠️ Review | Check if needed or dead code | 1. Check imports in handlers: `grep -r "from ['\"]\\./app" functions/src/` 2. Search for references: `grep -r "src/app" functions/src/` 3. If ZERO matches → mark for deletion 4. Otherwise → commit |
| `functions/src/openapi/openapi.ts` | ✅ Commit | OpenAPI generation | ✓ Verify implementation  ✓ Check integration  → `git add && git commit` |
| `functions/src/scheduled/` | ⚠️ Review | Check scheduled tasks | 1. Check cron/task config in Cloud Tasks 2. Check deployment manifests 3. If NOT deployed → document as WIP 4. Otherwise → commit |
| `functions/src/utils/helpers.ts` | ⚠️ Review | Check if duplicate of existing helpers | 1. Diff against existing .ts files in utils: `for file in functions/src/utils/*.ts; do if [[ "$file" != *"helpers.ts" ]]; then echo "Comparing with $file:"; diff functions/src/utils/helpers.ts "$file"; fi; done` 2. Detect duplicates 3. If duplicate → merge/replace existing 4. If new → commit |
...
1. [ ] `queries/tarkovDataQuery.ts`:
   ...
   - [ ] Maintains identical query structure
...
- [ ] Maintain identical API as before
...
### Each can be completed in < 1 hour
...
- [ ] **Remove console.log statements** - 30 min
  ...
  find functions/src -name "*.ts" -exec sed -i.bak 's/console\.log/functions.logger.info/g' {} \;
  find functions/src -name "*.ts" -exec sed -i.bak 's/console\.warn/functions.logger.warn/g' {} \;
  find functions/src -name "*.ts" -exec sed -i.bak 's/console\.error/functions.logger.error/g' {} \;
  find functions/src -name "*.ts" -exec sed -i.bak 's/console\.debug/functions.logger.debug/g' {} \;
